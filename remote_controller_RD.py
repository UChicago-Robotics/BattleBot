...
try:
    while running:
        ...
        if len(gamepads) > 0 and not paused:
            ...
            draw_stick_r.x = stick_r.y * math.sqrt(1 - 0.5 * stick_r.x ** 2)
            draw_stick_r.y = -stick_r.x * math.sqrt(1 - 0.5 * stick_r.y ** 2)
            ...
            pg.draw.rect(screen, color_light, [307, 400 + 88, 150, 2])
            pg.draw.rect(screen, color_dark, [381 - 75 * right_stick_gui, 481, 4, 15])
            screen.blit(rs_text, (382 - rs_text.get_width() / 2, 378))
            screen.blit(rs_text_value, (381 - rs_text_value.get_width() / 2, 400 + 89 - 32))
